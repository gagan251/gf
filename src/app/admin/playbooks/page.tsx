...
import { Loader2, UploadCloud, BookMarked, Trash2, Download } from 'lucide-react';
...
function UploadPlaybookForm({ onUploadComplete }: { onUploadComplete: () => void }) {
    ...
    async function onSubmit(values: z.infer<typeof uploadFormSchema>) {
        ...
        const storageRef = ref(storage, `playbooks/${Date.now()}-${file.name}`);
        ...
        uploadTask.on('state_changed',
            ...
            async () => {
                ...
                await addDoc(collection(firestore, 'playbooks'), {
                    name: file.name,
                    url: downloadURL,
                ...
                setIsLoading(false);
                setUploadProgress(null);
                toast({ title: "Upload Successful", description: `${file.name} has been added to the playbook.` });
                form.reset();
                onUploadComplete();
            }
        );
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><UploadCloud /> Upload to Playbook</CardTitle>
                <CardDescription>Add new playbooks for your students.</CardDescription>
            </CardHeader>
            <CardContent>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <Button type="submit" disabled={isLoading}>
                        ...
                        {isLoading ? `Uploading... ${uploadProgress?.toFixed(0)}%` : 'Upload Playbook'}
                    </Button>
                </form>
            </CardContent>
        </Card>
...
function PlaybooksList() {
    const firestore = useFirestore();
    const query = useMemoFirebase(() => firestore ? collection(firestore, 'playbooks') : null, [firestore]);
    const { data: files, isLoading } = useCollection<PlaybookFile>(query);
    const { toast } = useToast();
    ...
    return (
        <Card className="mt-8">
            <CardHeader>
                <CardTitle>Playbook Content</CardTitle>
            </CardHeader>
            <CardContent>
                {files && files.length > 0 ? (
                    ...
                    <p className="text-center text-muted-foreground py-8">No files in the playbook yet.</p>
                )}
            </CardContent>
        </Card>
    );
...
export default function AdminPlaybooksPage() {
    ...
    return (
        <AdminShell
            ...
            pageDescription="Upload and manage playbooks and other strategic documents."
            headerIcon={<BookMarked className="h-8 w-8 text-primary" />}
        >
            <div className="grid grid-cols-1 gap-8">
                <UploadPlaybookForm onUploadComplete={() => setKey(k => k + 1)} />
                <PlaybooksList key={key} />
            </div>
        </AdminShell>
    );
}
